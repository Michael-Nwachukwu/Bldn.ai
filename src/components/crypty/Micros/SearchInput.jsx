...
const SearchInput = () => {
    ...
    const handleUpdateInputMobile = (e) => {
        const value = e.target.value;
    ...
    const handleSubmitMobile = (event) => {
        ...
        if (inputMobile == ''){
            ...
            fetchDetails(inputMobile, baseUrl);
            console.log('Active Token:', inputMobile);
        ...
        <>
            ...
            <Box w={'70%'} ml={'auto'} pt={3} display={{ sm:'none' }} mb={3}>
                <form onSubmit={handleSubmitMobile} style={{ width:'100%' }} className="form-control">
                    ...
                    <a href="#" type='submit' onClick={handleSubmitMobile}>
                        <Search fill={'#464a4d'} />
                    </a>
                </form>
                ...
                {/* <form onSubmit={handleSubmitMobile} style={{ width:'100%' }}>
                    <InputGroup ml={'auto'} size='md' maxW={'xs'}>
                        <Input
                            ...
                            placeholder='Search'
                            color={useColorModeValue('brand.800', 'white')}
                            onChange={handleUpdateInputMobile}
                            value={inputMobile}
                            name={"password"}
                            _hover={{ border:'' }}
                            _placeholder={{ color: useColorModeValue('brand.800', '#dfe5ed') }}
